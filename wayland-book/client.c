...
static void wl_pointer_frame(void *data, struct wl_pointer *wl_pointer) {
  ...
  if (event->event_mask & POINTER_EVENT_ENTER) {
    printf("entered %f, %f ", wl_fixed_to_double(event->surface_x), wl_fixed_to_double(event->surface_y));
  }
  ...
  if(event->event_mask & POINTER_EVENT_MOTION) {
    printf("motion %f %f ", wl_fixed_to_double(event->surface_x), wl_fixed_to_double(event->surface_y));
  }

  if(event->event_mask & POINTER_EVENT_BUTTON) {
    char *state = event->state == WL_POINTER_BUTTON_STATE_RELEASED ? "released" : "pressed";
    printf("pointer button %d %s ", event->button, state);
  }
...
static struct touch_point *get_touch_point(struct client_state *client_state, int32_t id) {
...
static void wl_touch_frame(void *data, struct wl_touch *wl_touch)
{
  ...
  for (size_t i = 0; i < nmemb; ++i) {
    ...
    printf("point %d: ", touch->points[i].id);

    if (point->event_mask & TOUCH_EVENT_DOWN) {
      printf("down %f,%f ", wl_fixed_to_double(point->surface_x), wl_fixed_to_double(point->surface_y));
    }
    ...
    if (point->event_mask & TOUCH_EVENT_MOTION) {
      printf("motion %f,%f ", wl_fixed_to_double(point->surface_x), wl_fixed_to_double(point->surface_y));
    }

    if (point->event_mask & TOUCH_EVENT_SHAPE) {
      printf("shape %fx%f ", wl_fixed_to_double(point->major), wl_fixed_to_double(point->minor));
    }

    if (point->event_mask & TOUCH_EVENT_ORIENTATION) {
      printf("orientation %f ", wl_fixed_to_double(point->orientation));
    }
...
static void wl_seatc_cababilities(void *data, struct wl_seat *wl_seat, uint32_t capabilities) {
  (void)wl_seat;
  struct client_state *state = data;
  ...
  if (have_keyboard && state->wl_pointer == NULL) {
...
static const struct wl_seat_listener wl_seat_listener = {
  .capabilities = wl_seatc_cababilities,
  .name = wl_seat_name,
};
